...
float setaltitude(float h, float relatepressure, float temper)
{
  return  (float)relatepressure*pow(((float)1+((float)-0.0065/((float)temper+(float)273))*h),5.2518);
}
float getaltitude(float pressure, float relatepressure, float temper)
{
  float a = ((temper+((float)273))/((float)-0.0065))*(pow(((float)pressure/(float)relatepressure),(float)0.19039)-(float)1);
  if(a<=0.0)return 0;
  else
  {
    return a;
...
void read_barometer(void) {
 // uint32_t read_barometer() {
  ...
  if (barometer_counter == 1) {                                                 //When the barometer_counter variable is 1.
    if (temperature_counter == 0) {                                             //And the temperature counter is 0.
      ...
      Wire.requestFrom(MS5611_address, 3); 
       for (int8_t k=3-1; k>=0; k--){                                          //Poll 3 data bytes from the MS5611
       value |=  (uint32_t) Wire.read() << (8*k) ;} 
        uint32_t D2;
        dT = C[5];
        dT <<= 8;
        dT *= -1;
        dT += raw_temperature;
        T=2000+((uint64_t)dT*C[6])/8388608;
      ...
      raw_temperature_rotating_memory[average_temperature_mem_location]  =  value ;  
      raw_average_temperature_total += raw_temperature_rotating_memory[average_temperature_mem_location];
     ...
     // for (int8_t k=3-1; k>=0; k--)
      for (int8_t k=2; k>=0; k--){                                          //Poll 3 data bytes from the MS5611
       value |=  (uint32_t) Wire.read()<< (8*k);}
      raw_pressure=value;
  ...
  if (barometer_counter == 2) {                                                 //If the barometer_counter variable equals 2.
   ...
   /* 
    dT = raw_temperature-((uint32_t)C[5] * 256);
   // Serial.println(raw_pressure);
    OFF = (int64_t)C[2]*65536 + ((int64_t)dT * (int64_t)C[4]) /128;
    SENS =(int64_t)C[1]*32768  + ((int64_t)dT * (int64_t)C[3]) / 256;
    P = (raw_pressure * SENS/2097152 - OFF) / 32768; //*/
    dT = C[5];
    dT <<= 8;
    dT *= -1;
    dT += raw_temperature;
    ...
    pressure_total_average += pressure_rotating_mem[pressure_rotating_mem_location];                          //Add the new value to the long term avarage value.
    pressure_rotating_mem_location++;                                                                         //Increase the rotating memory location.
    if (pressure_rotating_mem_location == 40)pressure_rotating_mem_location = 0;                              //Start at 0 when the memory location 20 is reached.
    actual_pressure_fast = (float)pressure_total_average / 40.0;                                              //Calculate the average pressure of the last 20 pressure readings.

    //To get better results we will use a complementary fillter that can be adjusted by the fast average.
     actual_pressure_slow = actual_pressure_slow * (float)0.992 + actual_pressure_fast * (float)0.008;
   // actual_pressure_slow = actual_pressure_slow * (float)0.980 + actual_pressure_fast * (float)0.02;
    ...
    altitude_quad =(float)getaltitude(actual_pressure, ground_pressure, (float)T/100.0);
...
// }
 ...
 // if (barometer_counter == 3) {                                                                               //When the barometer counter is 3
    ...
    //  if (pid_altitude_setpoint == 0)pid_altitude_setpoint = actual_pressure;                                 //If not yet set, set the PID altitude setpoint.
      if (pid_altitude_setpoint == 0)pid_altitude_setpoint=setaltitude(stable_altitude, ground_pressure ,(float)temper);
  ...
  /*    if (receiver_input_channel_3 > 1680) {                                                        //If the throtttle is increased above 1600us (60%).
        manual_altitude_change = 1;                                                                 //Set the manual_altitude_change variable to 1 to indicate that the altitude is adjusted.      
       pid_altitude_setpoint = actual_pressure;                                                   //To prevent very fast changes in hight limit the function of the throttle.
        manual_throttle = (receiver_input_channel_3 - 1600) /4;  
      }
      if (receiver_input_channel_3 < 1500) {                                             //Adjust the setpoint to the actual pressure value so the output of the P- and I-controller are 0.
        ...
        manual_throttle = (receiver_input_channel_3 - 1400) /4 ;                                    //To prevent very fast changes in hight limit the function of the throttle.
      }   
     */
      ...
      if (pid_error_temp > 10 || pid_error_temp < -10) {                             //If the error between the setpoint and the actual pressure is larger than 10 or smaller then -10.
        pid_error_gain_altitude = (abs(pid_error_temp) - 10) / 20.0;                 //The positive pid_error_gain_altitude variable is calculated based on the error.
      ...
      pid_i_mem_altitude += (pid_i_gain_altitude / 100.0) * pid_error_temp; // default:(pid_i_gain_altitude / 100.0)
    ...
    else if ((flight_mode < 2 && pid_altitude_setpoint != 0) || ground_pressure == 0) {                        //If the altitude hold mode is not set and the PID altitude setpoint is still set.
